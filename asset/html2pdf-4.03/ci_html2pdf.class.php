...
/**
 * CI_HTML2PDF Librairy - main class
 *
 * HTML => PDF convertor
 * distributed under the LGPL License
 *
 * @author  Laurent MINGUET <webmaster@CI_HTML2PDF.fr>
 * @version 4.03
 */

if (!defined('__CLASS_CI_HTML2PDF__')) {

    define('__CLASS_CI_HTML2PDF__', '4.03');
    ...
    class CI_HTML2PDF
    {
        ...
        protected $_subPart          = false;       // sub CI_HTML2PDF
        ...
        protected $_isSubPart        = false;       // flag : in a sub CI_HTML2PDF
        ...
        protected $_isForOneLine     = false;       // flag : in a specific sub CI_HTML2PDF to have the height of the next line
        ...
        protected $_margeLeft        = 100;           // current marges of the page
        ...
        static protected $_subobj    = null;        // object CI_HTML2PDF prepared in order to accelerate the creation of sub CI_HTML2PDF
        ...
        /**
         * Clone to create a sub CI_HTML2PDF from CI_HTML2PDF::$_subobj
         *
         * @access public
         */
        ...
        {
            CI_HTML2PDF::$_subobj = null;
            CI_HTML2PDF::$_tables = array();
        }
        ...
        public function Output($name = '', $dest = false)
        {
            ...
            if (strtolower(substr($name, -4))!='.pdf') {
                throw new CI_HTML2PDF_exception(0, 'The output document name "'.$name.'" is not a PDF name');
            }
        ...
        /**
         * convert the HTML of a real page, to a code adapted to CI_HTML2PDF
         *
         * @access public
         * @param  string HTML of a real page
         * @return string HTML adapted to CI_HTML2PDF
         */
        ...
        /**
         * init a sub CI_HTML2PDF. does not use it directly. Only the method createSubHTML must use it
         *
         * @access public
         * @param  string  $format
        ...
        {
            $content = preg_replace('/<page_header([^>]*)>/isU', '<hr>'.CI_HTML2PDF_locale::get('vue01').' : $1<hr><div$1>', $content);
            $content = preg_replace('/<page_footer([^>]*)>/isU', '<hr>'.CI_HTML2PDF_locale::get('vue02').' : $1<hr><div$1>', $content);
            $content = preg_replace('/<page([^>]*)>/isU', '<hr>'.CI_HTML2PDF_locale::get('vue03').' : $1<hr><div$1>', $content);
...
<html>
    <head>
        <title>'.CI_HTML2PDF_locale::get('vue04').' HTML</title>
...
'.$content.'
        ...
        /**
         * prepare CI_HTML2PDF::$_subobj (used for create the sub CI_HTML2PDF objects
         *
         * @access protected
         */
        protected function _prepareSubObj()
        {
            ...
            // create the sub object
            CI_HTML2PDF::$_subobj = new CI_HTML2PDF(
                                        $this->_orientation,
                                        $this->_format,
                                        $this->_langue,
                                        $this->_unicode,
            ...
            // init
            CI_HTML2PDF::$_subobj->setTestTdInOnePage($this->_testTdInOnepage);
            CI_HTML2PDF::$_subobj->setTestIsImage($this->_testIsImage);
            CI_HTML2PDF::$_subobj->setTestIsDeprecated($this->_testIsDeprecated);
            CI_HTML2PDF::$_subobj->setDefaultFont($this->_defaultFont);
            CI_HTML2PDF::$_subobj->parsingCss->css            = &$this->parsingCss->css;
            CI_HTML2PDF::$_subobj->parsingCss->cssKeys        = &$this->parsingCss->cssKeys;

            // clone font from the original PDF
            CI_HTML2PDF::$_subobj->pdf->cloneFontFrom($this->pdf);

            // remove the link to the parent
            CI_HTML2PDF::$_subobj->parsingCss->setPdfParent($pdf);
        }

        /**
         * create a sub CI_HTML2PDF, to calculate the multi-tables
         *
         * @access protected
         * @param  &CI_HTML2PDF $subHtml sub CI_HTML2PDF to create
        ...
        protected function _createSubHTML(&$subHtml, $cellmargin=0)
        {
            ...
            CI_HTML2PDF::$_subobj->pdf->getPage();

            // clone the sub oject
            $subHtml = clone CI_HTML2PDF::$_subobj;
        ...
        protected function _executeAction($action)
        {
            ...
            if (!is_callable(array(&$this, $fnc))) {
                throw new CI_HTML2PDF_exception(1, strtoupper($action['name']), $this->parsingHtml->getHtmlErrorCode($action['html_pos']));
            }
        ...
        protected function _drawImage($src, $subLi=false)
        {
            ...
            if (count($infos)<2) {
                ...
                if ($this->_testIsImage) {
                    throw new CI_HTML2PDF_exception(6, $src);
                }
        ...
        protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
        {
            ...
            if ($background['image']) {
                ...
                if (count($imageInfos)<2) {
                    if ($this->_testIsImage) {
                        throw new CI_HTML2PDF_exception(6, $iName);
                    }
                } else {
        ...
        protected function _tag_open_NOBREAK($param)
        {
            ...
            // create a sub CI_HTML2PDF to execute the content of the tag, to get the dimensions
            $sub = null;
            $this->_createSubHTML($sub);
            $sub->parsingHtml->code = $this->parsingHtml->getLevel($this->_parsePos);
            $sub->_makeHTMLcode();
        ...
        protected function _tag_open_DIV($param, $other = 'div')
        {
            ...
            // create a sub CI_HTML2PDF to get the dimensions of the content of the div
            $w = 0; $h = 0;
            if (count($level)) {
                $sub = null;
                $this->_createSubHTML($sub);
        ...
        protected function _tag_open_BARCODE($param)
        {
            ...
            if ($this->_testIsDeprecated && (isset($param['bar_h']) || isset($param['bar_w'])))
                throw new CI_HTML2PDF_exception(9, array('BARCODE', 'bar_h, bar_w'));
        ...
        protected function _tag_open_QRCODE($param)
        {
            if ($this->_testIsDeprecated && (isset($param['size']) || isset($param['noborder'])))
                throw new CI_HTML2PDF_exception(9, array('QRCODE', 'size, noborder'));
            ...
            while ($x+$w>$right && $x<$right+$space && count($words)) {
                ...
                if (count($words)) {
                    ...
                    if ($nb>10000) {
                        ...
                        throw new CI_HTML2PDF_exception(2, array($txt, $right-$left, $w));
                    }
        ...
        protected function _tag_open_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                CI_HTML2PDF::$_tables[$param['num']]['thead']['tr'][0] = CI_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                CI_HTML2PDF::$_tables[$param['num']]['thead']['code'] = array();
                for ($pos=$this->_tempPos; $pos<count($this->parsingHtml->code); $pos++) {
                    ...
                    CI_HTML2PDF::$_tables[$param['num']]['thead']['code'][] = $action;
                    if (strtolower($action['name'])=='thead_sub' && $action['close']) break;
                }
            } else {
                $level = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos+= count($level);
                CI_HTML2PDF::$_tables[$param['num']]['tr_curr']+= count(CI_HTML2PDF::$_tables[$param['num']]['thead']['tr']);
            }
        ...
        protected function _tag_close_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                $min = CI_HTML2PDF::$_tables[$param['num']]['thead']['tr'][0];
                $max = CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1;
                CI_HTML2PDF::$_tables[$param['num']]['thead']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TFOOT($param)
        {
            ...
            if ($this->_subPart) {
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'][0] = CI_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']['code'] = array();
                for ($pos=$this->_tempPos; $pos<count($this->parsingHtml->code); $pos++) {
                    $action = $this->parsingHtml->code[$pos];
                    if (strtolower($action['name'])=='tfoot') $action['name'] = 'tfoot_sub';
                    CI_HTML2PDF::$_tables[$param['num']]['tfoot']['code'][] = $action;
                    if (strtolower($action['name'])=='tfoot_sub' && $action['close']) break;
                }
            } else {
                $level = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos+= count($level);
                CI_HTML2PDF::$_tables[$param['num']]['tr_curr']+= count(CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr']);
            }
        ...
        protected function _tag_close_TFOOT($param)
        {
            ...
            if ($this->_subPart) {
                $min = CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'][0];
                $max = CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1;
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TABLE($param, $other = 'table')
        {
            ...
            if ($this->_subPart) {
                ...
                CI_HTML2PDF::$_tables[$param['num']] = array();
                CI_HTML2PDF::$_tables[$param['num']]['border']          = isset($param['border']) ? $this->parsingCss->readBorder($param['border']) : null;
                CI_HTML2PDF::$_tables[$param['num']]['cellpadding']     = $this->parsingCss->ConvertToMM(isset($param['cellpadding']) ? $param['cellpadding'] : '1px');
                CI_HTML2PDF::$_tables[$param['num']]['cellspacing']     = $this->parsingCss->ConvertToMM(isset($param['cellspacing']) ? $param['cellspacing'] : '2px');
                CI_HTML2PDF::$_tables[$param['num']]['cases']           = array();          // properties of each TR/TD
                CI_HTML2PDF::$_tables[$param['num']]['corr']            = array();          // link between TR/TD and colspan/rowspan
                CI_HTML2PDF::$_tables[$param['num']]['corr_x']          = 0;                // position in 'cases'
                CI_HTML2PDF::$_tables[$param['num']]['corr_y']          = 0;                // position in 'cases'
                CI_HTML2PDF::$_tables[$param['num']]['td_curr']         = 0;                // current column
                CI_HTML2PDF::$_tables[$param['num']]['tr_curr']         = 0;                // current row
                CI_HTML2PDF::$_tables[$param['num']]['curr_x']          = $this->pdf->getX();
                CI_HTML2PDF::$_tables[$param['num']]['curr_y']          = $this->pdf->getY();
                CI_HTML2PDF::$_tables[$param['num']]['width']           = 0;                // global width
                CI_HTML2PDF::$_tables[$param['num']]['height']          = 0;                // global height
                CI_HTML2PDF::$_tables[$param['num']]['align']           = $alignObject;
                CI_HTML2PDF::$_tables[$param['num']]['marge']           = array();
                CI_HTML2PDF::$_tables[$param['num']]['marge']['t']      = $this->parsingCss->value['padding']['t']+$this->parsingCss->value['border']['t']['width']+CI_HTML2PDF::$_tables[$param['num']]['cellspacing']*0.5;
                CI_HTML2PDF::$_tables[$param['num']]['marge']['r']      = $this->parsingCss->value['padding']['r']+$this->parsingCss->value['border']['r']['width']+CI_HTML2PDF::$_tables[$param['num']]['cellspacing']*0.5;
                CI_HTML2PDF::$_tables[$param['num']]['marge']['b']      = $this->parsingCss->value['padding']['b']+$this->parsingCss->value['border']['b']['width']+CI_HTML2PDF::$_tables[$param['num']]['cellspacing']*0.5;
                CI_HTML2PDF::$_tables[$param['num']]['marge']['l']      = $this->parsingCss->value['padding']['l']+$this->parsingCss->value['border']['l']['width']+CI_HTML2PDF::$_tables[$param['num']]['cellspacing']*0.5;
                CI_HTML2PDF::$_tables[$param['num']]['page']            = 0;                // number of pages
                CI_HTML2PDF::$_tables[$param['num']]['new_page']        = true;             // flag : new page for the current TR
                CI_HTML2PDF::$_tables[$param['num']]['style_value']     = null;             // CSS style of the table
                CI_HTML2PDF::$_tables[$param['num']]['thead']           = array();          // properties on the thead
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']           = array();          // properties on the tfoot
                CI_HTML2PDF::$_tables[$param['num']]['thead']['tr']     = array();          // list of the TRs in the thead
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr']     = array();          // list of the TRs in the tfoot
                CI_HTML2PDF::$_tables[$param['num']]['thead']['height']    = 0;             // thead height
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']['height']    = 0;             // tfoot height
                CI_HTML2PDF::$_tables[$param['num']]['thead']['code'] = array();            // HTML content of the thead
                CI_HTML2PDF::$_tables[$param['num']]['tfoot']['code'] = array();            // HTML content of the tfoot
                CI_HTML2PDF::$_tables[$param['num']]['cols']        = array();              // properties of the COLs

                $this->_saveMargin($this->pdf->getlMargin(), $this->pdf->gettMargin(), $this->pdf->getrMargin());

                $this->parsingCss->value['width']-= CI_HTML2PDF::$_tables[$param['num']]['marge']['l'] + CI_HTML2PDF::$_tables[$param['num']]['marge']['r'];
            } else {
                // we start from the first page and the first page of the table
                CI_HTML2PDF::$_tables[$param['num']]['page'] = 0;
                CI_HTML2PDF::$_tables[$param['num']]['td_curr']    = 0;
                CI_HTML2PDF::$_tables[$param['num']]['tr_curr']    = 0;
                CI_HTML2PDF::$_tables[$param['num']]['td_x']        = CI_HTML2PDF::$_tables[$param['num']]['marge']['l']+CI_HTML2PDF::$_tables[$param['num']]['curr_x'];
                CI_HTML2PDF::$_tables[$param['num']]['td_y']        = CI_HTML2PDF::$_tables[$param['num']]['marge']['t']+CI_HTML2PDF::$_tables[$param['num']]['curr_y'];

                // draw the borders/background of the first page/part of the table
                $this->_drawRectangle(
                    CI_HTML2PDF::$_tables[$param['num']]['curr_x'],
                    CI_HTML2PDF::$_tables[$param['num']]['curr_y'],
                    CI_HTML2PDF::$_tables[$param['num']]['width'],
                    isset(CI_HTML2PDF::$_tables[$param['num']]['height'][0]) ? CI_HTML2PDF::$_tables[$param['num']]['height'][0] : null,
                    $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'],
                    0,
                    $this->parsingCss->value['background']
                ...
                CI_HTML2PDF::$_tables[$param['num']]['style_value'] = $this->parsingCss->value;
            }
        ...
        protected function _tag_close_TABLE($param)
        {
            ...
            if ($this->_subPart) {
                ...
                $this->_calculateTableCellSize(CI_HTML2PDF::$_tables[$param['num']]['cases'], CI_HTML2PDF::$_tables[$param['num']]['corr']);

                // calculate the height of the thead and the tfoot
                $lst = array('thead', 'tfoot');
                foreach ($lst as $mode) {
                    CI_HTML2PDF::$_tables[$param['num']][$mode]['height'] = 0;
                    foreach (CI_HTML2PDF::$_tables[$param['num']][$mode]['tr'] as $tr) {
                        // hauteur de la ligne tr
                        $h = 0;
                        for ($i=0; $i<count(CI_HTML2PDF::$_tables[$param['num']]['cases'][$tr]); $i++)
                            if (CI_HTML2PDF::$_tables[$param['num']]['cases'][$tr][$i]['rowspan']==1)
                                $h = max($h, CI_HTML2PDF::$_tables[$param['num']]['cases'][$tr][$i]['h']);
                        CI_HTML2PDF::$_tables[$param['num']][$mode]['height']+= $h;
                    }
                }

                // calculate the width of the table
                CI_HTML2PDF::$_tables[$param['num']]['width'] = CI_HTML2PDF::$_tables[$param['num']]['marge']['l'] + CI_HTML2PDF::$_tables[$param['num']]['marge']['r'];
                if (isset(CI_HTML2PDF::$_tables[$param['num']]['cases'][0])) {
                    foreach (CI_HTML2PDF::$_tables[$param['num']]['cases'][0] as $case) {
                        CI_HTML2PDF::$_tables[$param['num']]['width']+= $case['w'];
                    }
                }
                ...
                $old = $this->parsingCss->getOldValues();
                $parentWidth = $old['width'] ? $old['width'] : $this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin();
                $x = CI_HTML2PDF::$_tables[$param['num']]['curr_x'];
                $w = CI_HTML2PDF::$_tables[$param['num']]['width'];
                if ($parentWidth>$w) {
                    if (CI_HTML2PDF::$_tables[$param['num']]['align']=='center')
                        $x = $x + ($parentWidth-$w)*0.5;
                    else if (CI_HTML2PDF::$_tables[$param['num']]['align']=='right')
                        $x = $x + $parentWidth-$w;

                    CI_HTML2PDF::$_tables[$param['num']]['curr_x'] = $x;
                }

                // calculate the height of the table
                CI_HTML2PDF::$_tables[$param['num']]['height'] = array();

                // minimum of the height because of margins, and of the thead and tfoot height
                $h0 = CI_HTML2PDF::$_tables[$param['num']]['marge']['t'] + CI_HTML2PDF::$_tables[$param['num']]['marge']['b'];
                $h0+= CI_HTML2PDF::$_tables[$param['num']]['thead']['height'] + CI_HTML2PDF::$_tables[$param['num']]['tfoot']['height'];

                // max height of the page
                $max = $this->pdf->getH() - $this->pdf->getbMargin();

                // current position on the page
                $y = CI_HTML2PDF::$_tables[$param['num']]['curr_y'];
                $height = $h0;

                // we get the height of each line
                for ($k=0; $k<count(CI_HTML2PDF::$_tables[$param['num']]['cases']); $k++) {

                    // if it is a TR of the thead or of the tfoot => skip
                    if (in_array($k, CI_HTML2PDF::$_tables[$param['num']]['thead']['tr'])) continue;
                    if (in_array($k, CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'])) continue;

                    // height of the line
                    $th = 0;
                    $h = 0;
                    for ($i=0; $i<count(CI_HTML2PDF::$_tables[$param['num']]['cases'][$k]); $i++) {
                        $h = max($h, CI_HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['h']);

                        if (CI_HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['rowspan']==1)
                            $th = max($th, CI_HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['h']);
                    }

                    // if the row does not fit on the page => new page
                    if ($y+$h+$height>$max) {
                        if ($height==$h0) $height = null;
                        CI_HTML2PDF::$_tables[$param['num']]['height'][] = $height;
                        $height = $h0;
                        $y = $this->_margeTop;
                    }
                    $height+= $th;
                ...
                if ($height!=$h0 || $k==0) CI_HTML2PDF::$_tables[$param['num']]['height'][] = $height;
            } else {
                // if we have tfoor, draw it
                if (count(CI_HTML2PDF::$_tables[$param['num']]['tfoot']['code'])) {
                    $tmpTR = CI_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                    $tmpTD = CI_HTML2PDF::$_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

                    CI_HTML2PDF::$_tables[$param['num']]['tr_curr'] = CI_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'][0];
                    CI_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = CI_HTML2PDF::$_tables[$param['num']]['tfoot']['code'];
                    ...
                    $this->_parsePos =     $oldParsePos;
                    $this->parsingHtml->code = $oldParseCode;
                    CI_HTML2PDF::$_tables[$param['num']]['tr_curr'] = $tmpTR;
                    CI_HTML2PDF::$_tables[$param['num']]['td_curr'] = $tmpTD;
                }

                // get the positions of the end of the table
                $x = CI_HTML2PDF::$_tables[$param['num']]['curr_x'] + CI_HTML2PDF::$_tables[$param['num']]['width'];
                if (count(CI_HTML2PDF::$_tables[$param['num']]['height'])>1)
                    $y = $this->_margeTop+CI_HTML2PDF::$_tables[$param['num']]['height'][count(CI_HTML2PDF::$_tables[$param['num']]['height'])-1];
                else if (count(CI_HTML2PDF::$_tables[$param['num']]['height'])==1)
                    $y = CI_HTML2PDF::$_tables[$param['num']]['curr_y']+CI_HTML2PDF::$_tables[$param['num']]['height'][count(CI_HTML2PDF::$_tables[$param['num']]['height'])-1];
                else
                    $y = CI_HTML2PDF::$_tables[$param['num']]['curr_y'];
        ...
        protected function _tag_open_COL($param)
        {
            ...
            for ($k=0; $k<$span; $k++)
                CI_HTML2PDF::$_tables[$param['num']]['cols'][] = $param;
        }
        ...
        protected function _tag_open_TR($param, $other = 'tr')
        {
            ...
            CI_HTML2PDF::$_tables[$param['num']]['tr_curr']++;
            CI_HTML2PDF::$_tables[$param['num']]['td_curr']= 0;
            ...
            if (!$this->_subPart) {
                ...
                // Y after the row
                $ty=null;
                for ($ii=0; $ii<count(CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1]); $ii++) {
                    $ty = max($ty, CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][$ii]['h']);
                }

                // height of the tfoot
                $hfoot = CI_HTML2PDF::$_tables[$param['num']]['tfoot']['height'];

                // if the line does not fit on the page => new page
                if (!$this->_isInTfoot && CI_HTML2PDF::$_tables[$param['num']]['td_y'] + CI_HTML2PDF::$_tables[$param['num']]['marge']['b'] + $ty +$hfoot> $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    // new page
                    CI_HTML2PDF::$_tables[$param['num']]['new_page'] = true;
                    $this->_setNewPage();

                    // new position
                    CI_HTML2PDF::$_tables[$param['num']]['page']++;
                    CI_HTML2PDF::$_tables[$param['num']]['curr_y'] = $this->pdf->getY();
                    CI_HTML2PDF::$_tables[$param['num']]['td_y'] = CI_HTML2PDF::$_tables[$param['num']]['curr_y']+CI_HTML2PDF::$_tables[$param['num']]['marge']['t'];

                    // if we have the height of the tbale on the page => draw borders and background
                    if (isset(CI_HTML2PDF::$_tables[$param['num']]['height'][CI_HTML2PDF::$_tables[$param['num']]['page']])) {
                        $old = $this->parsingCss->value;
                        $this->parsingCss->value = CI_HTML2PDF::$_tables[$param['num']]['style_value'];

                        $this->_drawRectangle(
                            CI_HTML2PDF::$_tables[$param['num']]['curr_x'],
                            CI_HTML2PDF::$_tables[$param['num']]['curr_y'],
                            CI_HTML2PDF::$_tables[$param['num']]['width'],
                            CI_HTML2PDF::$_tables[$param['num']]['height'][CI_HTML2PDF::$_tables[$param['num']]['page']],
                            $this->parsingCss->value['border'],
                            $this->parsingCss->value['padding'],
                            CI_HTML2PDF::$_tables[$param['num']]['cellspacing']*0.5,
                            $this->parsingCss->value['background']
                        );
                ...
                if (CI_HTML2PDF::$_tables[$param['num']]['new_page'] && count(CI_HTML2PDF::$_tables[$param['num']]['thead']['code'])) {
                    CI_HTML2PDF::$_tables[$param['num']]['new_page'] = false;
                    $tmpTR = CI_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                    $tmpTD = CI_HTML2PDF::$_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

                    CI_HTML2PDF::$_tables[$param['num']]['tr_curr'] = CI_HTML2PDF::$_tables[$param['num']]['thead']['tr'][0];
                    CI_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = CI_HTML2PDF::$_tables[$param['num']]['thead']['code'];
            ...
            // else (in a sub HTML)
                ...
                // prepare it
                CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1] = array();
                if (!isset(CI_HTML2PDF::$_tables[$param['num']]['corr'][CI_HTML2PDF::$_tables[$param['num']]['corr_y']]))
                    CI_HTML2PDF::$_tables[$param['num']]['corr'][CI_HTML2PDF::$_tables[$param['num']]['corr_y']] = array();

                CI_HTML2PDF::$_tables[$param['num']]['corr_x']=0;
                while(isset(CI_HTML2PDF::$_tables[$param['num']]['corr'][CI_HTML2PDF::$_tables[$param['num']]['corr_y']][CI_HTML2PDF::$_tables[$param['num']]['corr_x']]))
                    CI_HTML2PDF::$_tables[$param['num']]['corr_x']++;
            }
        ...
        protected function _tag_close_TR($param)
        {
            ...
            if (!$this->_subPart) {
                ...
                $ty=null;
                for ($ii=0; $ii<count(CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1]); $ii++) {
                    if (CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][$ii]['rowspan']==1) {
                        $ty = CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][$ii]['h'];
                    }
                }

                // new position
                CI_HTML2PDF::$_tables[$param['num']]['td_x'] = CI_HTML2PDF::$_tables[$param['num']]['curr_x']+CI_HTML2PDF::$_tables[$param['num']]['marge']['l'];
                CI_HTML2PDF::$_tables[$param['num']]['td_y']+= $ty;
                CI_HTML2PDF::$_tables[$param['num']]['new_page'] = false;
            } else {
                CI_HTML2PDF::$_tables[$param['num']]['corr_y']++;
            }
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            $param['cellpadding'] = CI_HTML2PDF::$_tables[$param['num']]['cellpadding'].'mm';
            $param['cellspacing'] = CI_HTML2PDF::$_tables[$param['num']]['cellspacing'].'mm';
            ...
            $x = CI_HTML2PDF::$_tables[$param['num']]['td_curr'];
            $y = CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1;
            ...
            if (in_array($other, array('td', 'th'))) {
                // id of the column
                $numCol = isset(CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Xr']) ? CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Xr'] : CI_HTML2PDF::$_tables[$param['num']]['corr_x'];

                // we get the properties of the COL tag, if exist
                if (isset(CI_HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {

                    $colParam = CI_HTML2PDF::$_tables[$param['num']]['cols'][$numCol];

                    // for colspans => we get all the neede widths
                    $colParam['style']['width'] = array();
                    for ($k=0; $k<$colspan; $k++) {
                        if (isset(CI_HTML2PDF::$_tables[$param['num']]['cols'][$numCol+$k]['style']['width'])) {
                            $colParam['style']['width'][] = CI_HTML2PDF::$_tables[$param['num']]['cols'][$numCol+$k]['style']['width'];
                        }
                    }
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if (CI_HTML2PDF::$_tables[$param['num']]['border']) {
                    $legacy['border'] = array();
                    $legacy['border']['l'] = CI_HTML2PDF::$_tables[$param['num']]['border'];
                    $legacy['border']['t'] = CI_HTML2PDF::$_tables[$param['num']]['border'];
                    $legacy['border']['r'] = CI_HTML2PDF::$_tables[$param['num']]['border'];
                    $legacy['border']['b'] = CI_HTML2PDF::$_tables[$param['num']]['border'];
                }
            }
            ...
            if ($collapse) {
                if (!$this->_subPart) {
                    if (
                        (CI_HTML2PDF::$_tables[$param['num']]['tr_curr']>1 && !CI_HTML2PDF::$_tables[$param['num']]['new_page']) ||
                        (!$this->_isInThead && count(CI_HTML2PDF::$_tables[$param['num']]['thead']['code']))
                    ) {
                        $this->parsingCss->value['border']['t'] = $this->parsingCss->readBorder('none');
                    }
                }

                if (CI_HTML2PDF::$_tables[$param['num']]['td_curr']>0) {
            ...
            $marge = array();
            $marge['t'] = $this->parsingCss->value['padding']['t']+0.5*CI_HTML2PDF::$_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['t']['width'];
            $marge['r'] = $this->parsingCss->value['padding']['r']+0.5*CI_HTML2PDF::$_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['r']['width'];
            $marge['b'] = $this->parsingCss->value['padding']['b']+0.5*CI_HTML2PDF::$_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['b']['width'];
            $marge['l'] = $this->parsingCss->value['padding']['l']+0.5*CI_HTML2PDF::$_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['l']['width'];

            // if we are in a sub HTML
            if ($this->_subPart) {
                // new position in the table
                CI_HTML2PDF::$_tables[$param['num']]['td_curr']++;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x] = array();
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'] = 0;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'] = 0;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['dw'] = 0;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['colspan'] = $colspan;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['rowspan'] = $rowspan;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Xr'] = CI_HTML2PDF::$_tables[$param['num']]['corr_x'];
                CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Yr'] = CI_HTML2PDF::$_tables[$param['num']]['corr_y'];

                // prepare the mapping for rowspan and colspan
                for ($j=0; $j<$rowspan; $j++) {
                    for ($i=0; $i<$colspan; $i++) {
                        CI_HTML2PDF::$_tables[$param['num']]['corr']
                            [CI_HTML2PDF::$_tables[$param['num']]['corr_y']+$j]
                            [CI_HTML2PDF::$_tables[$param['num']]['corr_x']+$i] = ($i+$j>0) ? '' : array($x,$y,$colspan,$rowspan);
                    }
                }
                CI_HTML2PDF::$_tables[$param['num']]['corr_x']+= $colspan;
                while (isset(CI_HTML2PDF::$_tables[$param['num']]['corr'][CI_HTML2PDF::$_tables[$param['num']]['corr_y']][CI_HTML2PDF::$_tables[$param['num']]['corr_x']])) {
                    CI_HTML2PDF::$_tables[$param['num']]['corr_x']++;
                }
                ...
                // new position in the table
                CI_HTML2PDF::$_tables[$param['num']]['td_curr']++;
                CI_HTML2PDF::$_tables[$param['num']]['td_x']+= CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['dw'];

                // borders and background of the TD
                $this->_drawRectangle(
                    CI_HTML2PDF::$_tables[$param['num']]['td_x'],
                    CI_HTML2PDF::$_tables[$param['num']]['td_y'],
                    CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'],
                    CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'],
                    $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'],
                    CI_HTML2PDF::$_tables[$param['num']]['cellspacing']*0.5,
                    $this->parsingCss->value['background']
                ...
                $this->parsingCss->value['width'] = CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'] - $marge['l'] - $marge['r'];

                // marges = size of the TD
                $mL = CI_HTML2PDF::$_tables[$param['num']]['td_x']+$marge['l'];
                $mR = $this->pdf->getW() - $mL - $this->parsingCss->value['width'];
                $this->_saveMargin($mL, 0, $mR);

                // position of the content, from vertical-align
                $hCorr = CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'];
                $hReel = CI_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['real_h'];
                switch($this->parsingCss->value['vertical-align'])
                {
                    case 'bottom':
                        $yCorr = $hCorr-$hReel;
                ...
                $x = CI_HTML2PDF::$_tables[$param['num']]['td_x']+$marge['l'];
                $y = CI_HTML2PDF::$_tables[$param['num']]['td_y']+$marge['t']+$yCorr;
                $this->pdf->setXY($x, $y);
                $this->_setNewPositionForNewLine();
            }
        ...
        protected function _tag_close_TD($param)
        {
            ...
            if ($this->_subPart) {
                ...
                if ($this->_testTdInOnepage && $this->_subHtml->pdf->getPage()>1) {
                    throw new CI_HTML2PDF_exception(7);
                }
                ...
                CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][CI_HTML2PDF::$_tables[$param['num']]['td_curr']-1]['w'] = max(array($w0, $w2));
                CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][CI_HTML2PDF::$_tables[$param['num']]['td_curr']-1]['h'] = max(array($h0, $h2));

                // real position of the content
                CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][CI_HTML2PDF::$_tables[$param['num']]['td_curr']-1]['real_w'] = $w0;
                CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][CI_HTML2PDF::$_tables[$param['num']]['td_curr']-1]['real_h'] = $h0;
                ...
                CI_HTML2PDF::$_tables[$param['num']]['td_x']+= CI_HTML2PDF::$_tables[$param['num']]['cases'][CI_HTML2PDF::$_tables[$param['num']]['tr_curr']-1][CI_HTML2PDF::$_tables[$param['num']]['td_curr']-1]['w'];
            }
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'LINE');
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'RECT');
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'CIRCLE');
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'ELLIPSE');
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'POLYGON');
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'PATH');
        ...
        {
            if (!$this->_isInDraw) throw new CI_HTML2PDF_exception(8, 'G');
